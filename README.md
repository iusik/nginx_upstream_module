...
This paragraph is actually only if you chose [build from source](#build-from-source)
installation. So that, nginx git repository should be placed into
`nginx` directory within the module repository.

Firstly, run all tests with `./test/auto.sh`. Before test running
itself this script creates necessary environment for testing. It
will checkout several nginx versions, rebuild nginx with the
module and run tests on each of those nginx versions.
